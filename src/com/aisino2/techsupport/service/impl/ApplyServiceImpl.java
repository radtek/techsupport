...
public class ApplyServiceImpl extends BaseService implements ApplyService {
	...
	public String insertApplyAndGo(SupportTicket st){
		...
		try {
			...
			try {
				...
				RuntimeException a = new RuntimeException("角色["
						+ Constants.ST_ROLE_NAME_CE + "] 未创建..");
				log.error(a, a.fillInStackTrace());
				throw a;
			}
			User_role ceUser_role = new User_role();
			ceUser_role.setRoleid(cerole.getRoleid());
			List<User_role> ceUserRoleList = userRoleService
					.getListUser_role(ceUser_role);
			...
			if (ceUserRoleList != null && ceUserRoleList.size() > 0) {
				...
				ceApprovalUsersID = ceApprovalUsersIDBuffer.substring(0,
						ceApprovalUsersIDBuffer.length() - 1);
			}
			...
			if (supportTicket.getSupportDeptList() != null
				...
				// 使用新版的指派机构
				String departmentApprovalUsersId = "";
				for (Department supportDepartment : supportTicket
						.getSupportDeptList()) {
					User_role userRole = new User_role();

					try {
						if (util.getTechSupportEnvConfig(
								"techsupport.worksheet.approval_department.tech")
								.equals(supportDepartment.getDepartcode())) {
							// 指派技术开发部
							Role role = new Role();
							role.setRolename(Constants.ST_ROLE_TECH_DEPT_APPR_TYPE);
							role = (Role) roleService.getListRole(role).get(0);
							userRole.setRoleid(role.getRoleid());

						} else if (util
								.getTechSupportEnvConfig(
										"techsupport.worksheet.approval_department.product")
								.equals(supportDepartment.getDepartcode())) {
							// 指派产品方案部
							Role role = new Role();
							role.setRolename(Constants.ST_ROLE_PRODUCT_DEPT_APPR_TYPE);
							role = (Role) roleService.getListRole(role).get(0);
							userRole.setRoleid(role.getRoleid());
						}
					} catch (IOException e) {
					...
					if (userRole.getRoleid() == null
						...
						throw new RuntimeException(
								"指派负责部门发生错误，无法获取到负责部门对应的角色信息");
					List<User_role> lAssigneeUsers = userRoleService
							.getListUser_role(userRole);
					for (User_role ur : lAssigneeUsers) {
						departmentApprovalUsersId += ur.getUserid() + ",";
					}
				}
				departmentApprovalUsersId = departmentApprovalUsersId.length() > 0 ? departmentApprovalUsersId
						.substring(0, departmentApprovalUsersId.length() - 1)
						: "";
				candidateUsers.put("deptApprovalUsers",
						departmentApprovalUsersId);
			}
			...
			dictitem = dictitemService
					.getDictionaryItemByDictCodeAndFactValue(dictitem);
			...
			try {
				feedback_role = (Role) roleService.getListRole(feedback_role)
						.get(0);
			} catch (Exception e) {
				throw new RuntimeException("角色["
						+ Constants.ST_ROLE_NAME_FEEDBACKER + "]不存在");
			}
			...
			if (rgmRoleList != null && rgmRoleList.size() > 0) {
				...
				List<User_role> lstUserRole = userRoleService
						.getListUser_role(userrole);
			...
			try {
				...
				throw new RuntimeException("角色[" + Constants.ST_ROLE_NAME_QC
						+ "] 未创建..");
			}
			...
			if (userRoleList != null && userRoleList.size() > 0) {
				...
				archiverUserID = archiverUserIDBuffer.substring(0,
						archiverUserIDBuffer.length() - 1);
			}
			...
			// 流程启动
			ProcessInstance newpi = workflow.workflowStart(workflow
					.setVariable(null, null, candidateUsers, params));
			return newpi.getId();
	 ...
	 * 
	 * @see
	 * com.aisino2.techsupport.service.ApplyService#checkStNoIsExist(java.lang
	 * .String)
	 */
